...
fn serialization() {
    ...
    let v_circuit = MyCircuit {
        constant,
        a: Value::unknown(),
        b: Value::unknown(),
    };

    // Construct the VK and PK
    let params = Params::new(k);
    let vk = keygen_vk(&params, &v_circuit).expect("vk should not fail");
    let pk = keygen_pk(&params, vk.clone(), &v_circuit).expect("pk should not fail");
    ...
    let pk = ProvingKey::read::<_, MyCircuit<Fp>>(
        &mut reader,
    ...
    let mut reader = BufReader::new(f);
    let vk = VerifyingKey::read::<_, MyCircuit<Fp>>(
        &mut reader,
        SerdeFormat::RawBytes,
        #[cfg(feature = "circuit-params")]
        circuit.params(),
    ...
    // Prove & Verify using the serialized keys
